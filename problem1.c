...
int main(int argc, char const *argv[])
{
    ...
    {
        if (distance(intersection[i]) < min)
        {
            min = distance(intersection[i]);
        }
        printf("(%d, %d) -> %d\n", intersection[i].x, intersection[i].y, \
            distance(intersection[i]));
    }

    printf("Distance of closet intersection to (0, 0): %d\n", min);
...
struct coord * trace(char **instruction)
{
    ...
    for (size_t i = 0; instruction[i]; i++)
    {
        ...
        for (size_t i = 0; i < value; i++)
        {
            ...
            if (index == 0) { current.x = 0; current.y = 0; }
